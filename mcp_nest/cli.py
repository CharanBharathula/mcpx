...
def install(ctx: click.Context, server_name: str, clients: tuple) -> None:
    ...
    if not server:
        ...
        console.print(f"Run [bold]mcp-nest search {server_name}[/bold] to find available servers.")
        sys.exit(1)
...
def search(query: str) -> None:
    ...
    console.print(table)
    console.print(
        f"\n[dim]Found {len(results)} server(s). Use [bold]mcp-nest info <name>[/bold] for details.[/dim]"
    )
...
def list_servers(client: str) -> None:
    ...
    if not installed:
        ...
        console.print("Run [bold]mcp-nest install <server>[/bold] to get started.")
        return
...
def doctor() -> None:
    ...
    table = Table(title="mcp-nest Doctor", show_header=True, header_style="bold blue")
...
def profile_list() -> None:
    ...
    console.print(table)
    console.print(
        "\n[dim]Use [bold]mcp-nest profile install <name>[/bold] to install a profile.[/dim]"
    )
...
def profile_show(profile_name: str) -> None:
    ...
    if profile_name not in profiles:
        ...
        console.print("Run [bold]mcp-nest profile list[/bold] to see available profiles.")
        sys.exit(1)
...
def backup(show_list: bool) -> None:
    """Backup all client MCP configs to ~/.mcp-nest/backups/."""
    console = _console()
    config_manager = ConfigManager()
...
def restore(timestamp: str | None) -> None:
    """Restore client configs from a backup. Use 'mcp-nest backup --list' to see backups."""
    console = _console()
    config_manager = ConfigManager()
    backups = config_manager.list_backups()

    if not backups:
        console.print("[yellow]No backups found.[/yellow] Run [bold]mcp-nest backup[/bold] first.")
        return
    ...
    if timestamp:
        ...
        if not backup_dir:
            ...
            console.print("Run [bold]mcp-nest backup --list[/bold] to see available backups.")
            sys.exit(1)
    else:
...
def registry_group() -> None:
    """Manage the mcp-nest server registry."""
    pass
...
def registry_update() -> None:
    ...
    try:
        ...
        console.print("[dim]Run [bold]mcp-nest search[/bold] to see all available servers.[/dim]")
    except Exception as e:
